...
func (b *binder) Bind(i interface{}, c Context) (err error) {
	...
	case strings.HasPrefix(ctype, MIMEApplicationXML), strings.HasPrefix(ctype, "text/xml"):
...
func (b *binder) bindData(ptr interface{}, data map[string][]string) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if !exists {
			inputValue, exists = data[strings.ToLower(inputFieldName[:1])+inputFieldName[1:]]
			if !exists {
				continue
			}
		}
